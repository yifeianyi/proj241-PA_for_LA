...
static int find_callfun(paddr_t dnpc){
    int i;
    for(i = 0; i < fun_index; i ++){
        if(fun_list[i].fun_addr == dnpc){
...
void f_trace_call(paddr_t pc, paddr_t dnpc){
    if(S_table == NULL) return ;
    ++Fun_depth;

    int i = find_callfun(dnpc);
    char *fun_call_name;
    if (i >= 0 && i < fun_index) {
