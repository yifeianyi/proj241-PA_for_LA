...
#define  GRLEN 32
  ...
  INSTPAT("0000001010 ???????????? ????? ?????"   , ADDI.w   , 2RI12 , R(rd) = src1 + SEXT(imm,GRLEN));//Log("R(%d):%08x, imm:%08x, R(X)=%08x",rd,R(rd),imm,imm+R(rd));
  ...
  INSTPAT("0000001001 ???????????? ????? ?????"   , SLTUI    , 2RI12 , if((unsigned)src1 < (unsigned)imm) R(rd) =1);
  ...
  INSTPAT("0000000000 0101110????? ????? ?????"   , SLL.W    , 3R      , R(rd) = src1 << src2);
  ...
  INSTPAT("0000000001 0010001????? ????? ?????"   , SRAI.W   , 2RUI5    , R(rd) = SEXT((src1 >> (uint8_t)imm),32));

  INSTPAT("0000000000 0101010????? ????? ?????"   , or       , 3R    , R(rd) = src1 | src2);
  INSTPAT("0000000000 0101011????? ????? ?????"   , xor      , 3R    , R(rd) = src1 ^ src2);
  INSTPAT("0000000000 0100100????? ????? ?????"   , SLT      , 3R    , if(src1 < src2) R(rd) = 1);
  INSTPAT("0000000000 0100101????? ????? ?????"   , SLTU     , 3R    , if((unsigned)src1 < (unsigned)src2) R(rd) = 1);
  ...
  INSTPAT("010011???? ???????????? ????? ?????"   , jirl     , 2RI16 , R(rd) = s->pc + 4; s->dnpc = src1 + imm);
  ...
  INSTPAT("011000???? ???????????? ????? ?????"   , BLT      , 2RI16 , if(R(rd) > src1)  s->dnpc = s->pc + SEXT(imm<<2,GRLEN));
  INSTPAT("011001???? ???????????? ????? ?????"   , BGE      , 2RI16 , if(R(rd) <= src1)  s->dnpc = s->pc + SEXT(imm<<2,GRLEN));
  INSTPAT("011010???? ???????????? ????? ?????"   , BLTU     , 2RI16 , if((uint32_t)R(rd) > (uint32_t)src1) s->dnpc = s->pc + SEXT(imm<<2,GRLEN));
  INSTPAT("011011???? ???????????? ????? ?????"   , BGEU     , 2RI16 , if((uint32_t)R(rd) <= (uint32_t)src1) s->dnpc = s->pc + SEXT(imm<<2,GRLEN));
