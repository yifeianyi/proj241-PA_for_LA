...
#define  GRLEN 32
  ...
  INSTPAT("0000001010 ???????????? ????? ?????"   , ADDI.w   , 2RI12 , R(rd) = src1 + (signed)imm);//Log("R(%d):%08x, imm:%08x, R(X)=%08x",rd,R(rd),imm,imm+R(rd));
  INSTPAT("0000000000 0101010 ????? ????? ?????"  , or       ,  3R   , R(rd) = src1 | src2);
  INSTPAT("0000001110 ???????????? ????? ?????"   , ORI      , 2RUI12 , R(rd) = src1 | imm);
  INSTPAT("0000001101 ???????????? ????? ?????"   , ANDI     , 2RUI12 , R(rd) = src1 & BITS(imm,11,0));
  ...
  INSTPAT("0000001001 ???????????? ????? ?????"   , SLTUI    , 2RI12 , R(rd) = src1 < imm ? 1:0);
  INSTPAT("0000001001 ???????????? ????? ?????"   , SLTI     , 2RI12 , R(rd) =(signed)src1 < (signed)imm ? 1:0);
  
  
  INSTPAT("0000001101 ???????????? ????? ?????"   , XORI     , 2RUI12 , R(rd) = src1 ^ imm);
  ...
  INSTPAT("010011???? ???????????? ????? ?????"   , jirl     , 2RI16 , R(rd) = s->pc + 4; s->dnpc = src1 + (imm<<2));
  INSTPAT("010110???? ???????????? ????? ?????"   , BEQ      , 2RI16 , if(R(rd) == src1) s->dnpc = s->pc + (imm<<2));
  INSTPAT("010111???? ???????????? ????? ?????"   , BNE      , 2RI16 , if(R(rd) != src1) s->dnpc = s->pc + (imm<<2));
  INSTPAT("011000???? ???????????? ????? ?????"   , BLT      , 2RI16 , if(R(rd) > src1)  s->dnpc = s->pc + (imm<<2));
  INSTPAT("011001???? ???????????? ????? ?????"   , BGE      , 2RI16 , if(R(rd) <= src1) s->dnpc = s->pc +(imm<<2));
