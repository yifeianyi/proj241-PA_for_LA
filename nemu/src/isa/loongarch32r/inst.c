...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001101 ???????????? ????? ?????"   , ANDI     , 2RI12_U , R(rd) = src1 & (uint32_t)imm);
  INSTPAT("0000001110 ???????????? ????? ?????"   , ORI      , 2RI12_U , R(rd) = src1 | (uint32_t)imm);
  INSTPAT("0000001101 ???????????? ????? ?????"   , XORI     , 2RI12_U , R(rd) = src1 ^ (uint32_t)imm);
  ...
  INSTPAT("0000000001 0000001????? ????? ?????"   , SLLI.W   , 2R_U    , R(rd) = SEXT(((uint32_t)src1 << (uint8_t)imm),32));
  INSTPAT("0000000001 0001001????? ????? ?????"   , SRLI.W   , 2R_U    , R(rd) = SEXT(((uint32_t)src1 >> (uint8_t)imm),32));
  INSTPAT("0000000001 0001001????? ????? ?????"   , SRAI.W   , 2R_U    , R(rd) = SEXT((src1 >> (uint8_t)imm),32));
